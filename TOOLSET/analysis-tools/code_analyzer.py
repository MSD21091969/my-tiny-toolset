...
def create_tool_outputs_readme(output_base: str = ".tool-outputs", version: str = "current"):
    """Create README.md in .tool-outputs/ explaining contents"""
    readme_path = Path(output_base) / "README.md"
...
- **`analysis/`** - JSON/YAML/CSV analysis data (models, functions, endpoints)
...
### Current Analysis
- Models: Check `analysis/*.csv` for model lists
- Functions: Check `analysis/functions_*.csv`
- Version: Check `analysis/version_analysis.json`

### Regenerate
```powershell
# Full analysis
python $env:MY_TOOLSET\\version_tracker.py . --version {version} --json --yaml

# Quick analysis
python $env:MY_TOOLSET\\code_analyzer.py . --json --csv

# Excel export
python $env:MY_TOOLSET\\excel_exporter.py . --output excel/analysis.xlsx
```

---

**Note:** This folder is gitignored. Copy files elsewhere before cleanup.
"""
...
def main():
    ...
    # Create README in .tool-outputs/
    if args.csv or args.json:
        output_base = Path(args.output_dir).parent
        create_tool_outputs_readme(str(output_base))
